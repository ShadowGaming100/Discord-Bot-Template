...
class DiscordLogger {
  ...
  async send({ client, type = 'error', title, description, fields, embed, error }) {
    ...
    const finalFields = Array.isArray(fields) ? fields.map(f => ({
      ...f,
  ...
  error(opts)   {
    return this.send({ ...opts, type: 'error' });
  }
  command(opts) {
    return this.send({ ...opts, type: 'command' });
  }
  guild(opts)   {
    return this.send({ ...opts, type: 'guild' });
  }
  client(opts)  {
    return this.send({ ...opts, type: 'client' });
  }
  ban(opts)     {
    return this.send({ ...opts, type: 'ban' });
  }
  server(opts)  {
    return this.send({ ...opts, type: 'server' });
  }
}
